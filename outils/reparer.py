...
def d_balises(txt):
    """Balises <style>/<script> non refermees : tout le reste de la page
...
def d_accolades(txt):
    """Erreur de syntaxe dans un bloc <script>.

    Priorité à node --check (un vrai analyseur JavaScript) quand Node est
    disponible : fiable, sans faux positif. Sur les scripts denses de ce
    projet, un simple comptage d'accolades se fait piéger par des motifs
    valides (regex, objets imbriqués) et signale des défauts qui n'existent
    pas — défaut déjà rencontré, corrigé ici plutôt que contourné.
    Repli sur un comptage approché uniquement si node est introuvable."""
    constats = []
        ...
        if node:
            ...
            try:
                ...
                if r.returncode != 0:
                    ...
                    ligne_erreur = next(
                        (l for l in detail if "Error" in l), None
                    )
                    resume = ligne_erreur or (detail[0] if detail else "erreur de syntaxe")
                    constats.append(f"script #{i + 1} : {resume.strip()}")
            finally:
                os.unlink(chemin)
        else:
            sans = re.sub(r'"(?:\\.|[^"\\])*"|\'(?:\\.|[^\'\\])*\'|`(?:\\.|[^`\\])*`'
            ...
            if ecart:
                constats.append(
                    f"script #{i + 1} : {ecart:+d} accolade(s) non appariée(s) "
                    "(comptage approché — installez node pour un résultat fiable)"
                )
    return constats
...
def d_intervalles(txt):
    ...
    for m in re.finditer(r"(.{0,40})setInterval\([^,]{0,80},\s*(\d{1,4})\s*\)", txt):
        ...
        if not ms or ms >= 200:
            continue
        # un minuteur dont l'identifiant est conserve sera arrete plus tard
